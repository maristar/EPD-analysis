...
% This version creates only individual reports!!!15 mars21
%
clear all 
close all;
...
for o=1:global_N_File %Loop start 
    ...
    %% Create a report for each employee
    rpt_ansatte_temp=Report([ansatte_navn], 'pdf');
    open(rpt_ansatte_temp)
    % First page 
    T=Text("EPD rapport 01 Januar 2020 - 27 november 2020.");
    T.Bold=true;
	 T.FontSize="28pt";
    add(rpt_ansatte_temp, T);
    
    %% Create chapters for each employee
    ch=Chapter;
    ch.Title=ansatte_navn;
    ch.Numbered=false; 
    add(rpt_ansatte_temp, ch);
...
%% Individuelt rapport 
para_time=Paragraph([global_list(o).name(1:end-4) '. Total days in 2020: ' num2str(number_of_2020days)])    
add(rpt_ansatte_temp, para_time);
  ...
  %% Add to individual rapport
para_Hp07=['Total Sum DOse Hp07 (mSv):', num2str(Siste_Hp07)];
add(rpt_ansatte_temp,para_Hp07);


  para_Hp10=['Total Sum DOse Hp10 (mSv):', num2str(Siste_Hp10)];
 add(rpt_ansatte_temp,para_Hp10); %Paragraph(sprintf('Total Sum DOse Hp07 (mSv):', Siste_Hp07)));
  

  %% end examples add paragraph
...
%      xticklabels({(string_labels)}
     ...
     cd(global_raw_path)
     close(rpt_ansatte_temp);
     clear para_Hp07 para_Hp10 para_time ch
end
% close(fig1)

clear global_N_File o ansatte_navn var10 var11 str_text 
%close(fig); close(fig100); 
clear fig fig100 fig1
clear rpt_ansatte_temp
clear para_Hp07 para_Hp10 
...
%         dataHp07(:,j)=cell2mat(T2.Hp07(j));
  ...
  % End normalize
   ...
   % we do it only for Hp07
    ...
    %% Add to individual rapport
    para_time=Paragraph([global_list_2(o).name '. Total days in 2020: ' num2str(number_of_2020days)])    
    add(rpt_ansatte_temp, para_time);
  ...
  %% Add to individual rapport
    para_Hp07=['Total Sum DOse Hp07 (mSv):', num2str((1/1000)*Siste_Hp07)];
    add(rpt_ansatte_temp,para_Hp07);


    para_Hp10=['Total Sum DOse Hp10 (mSv):', num2str((1/1000)*Siste_Hp10)];
    add(rpt_ansatte_temp,para_Hp10); 
...
% ansatte_navn='Agnell Kristina'
% rpt=Report([ansatte_navn], 'pdf');
% open(rpt_ansatte_temp)
% ch=Chapter;
